...
def plot_groups(groups_cost, groups_rt, out_dir, show=False, series=None):
	...
	plt.xlabel('v (problem size)')
	plt.ylabel('Cost (average over tests)')
	plt.title('TSP: Average Cost vs V (comparison)')
	plt.legend()
	plt.grid(True)
	out = os.path.join(out_dir, 'cost_compare_avg_vs_v.png')
	plt.tight_layout()
	plt.savefig(out)
	if show:
		plt.show()
	...
	plt.title('TSP: Average Runtime vs V (comparison)')
	plt.legend()
	plt.grid(True)
	out = os.path.join(out_dir, 'runtime_compare_avg_vs_v.png')
	plt.tight_layout()
	plt.savefig(out)
	if show:
		plt.show()
